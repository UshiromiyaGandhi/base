...
/** @var common\models\Shop $shopModel */
/** @var \mdm\admin\models\User $userModel */
/** @var \backend\models\ShopProfileUploadForm $profileUploadForm */
/** @var \backend\models\ShopQrisUploadForm $shopQrisUploadForm */

$this->title = 'My Shop';
$this->params['breadcrumbs'][] = ['label' => 'Shops', 'url' => ['index']];
$this->params['breadcrumbs'][] = ['label' => $shopModel->name, 'url' => ['view', 'id' => $shopModel->id]];
...
{error}
				...
				<div class="mb-5.5">
					<?= $form->field($shopModel, 'name', array(
						'labelOptions' => array(
							'class' => 'mb-3 block text-sm font-medium text-black dark:text-white'
						),
						'inputOptions' => array(
...
{error}
                            "
					))->textInput(['maxlength' => true])->label('Shop Name') ?>
				</div>


				<div class="mb-5.5">
					<?= $form->field($shopModel, 'description', ['labelOptions' => [
						'class' => 'mb-3 block text-sm font-medium text-black dark:text-white'
					], 'inputOptions' => [
						'class' => 'w-full rounded border border-stroke bg-gray py-3 pl-11.5 pr-4.5 font-medium text-black focus:border-primary focus-visible:outline-none dark:border-strokedark dark:bg-meta-4 dark:text-white dark:focus:border-primary'
					], 'template' => "{label}
...
{hint}
{error}
                            "])->textarea(['maxlength' => true])->label("Shop's Description") ?>
				</div>
	...
	<div class="col-span-5 xl:col-span-2 gap-5">
		<div
			class="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark"
		>
			<div
			...
			<div class="p-7">
				<div class="mb-4 flex items-center gap-3">
					<div class="h-14 w-14 rounded-full">
						<img
							src=<?= 'uploads/profilephoto/' . $userModel->getShop()->profileimage ?>
							alt="User"/>
					</div>
					<div>
						<span
				...
				<?= $form2->field($profileUploadForm, 'imageFile', [
					'inputOptions' => [
		...
		<p class="text-sm font-medium">
			(max, 800 X 800px)
				...
				<div class="flex justify-end gap-4.5">
					...
					<?= Html::submitButton('Bos', ['class' => 'flex justify-center rounded bg-primary px-6 py-2 font-medium text-gray hover:bg-opacity-90']) ?>
				</div>
				<?php ActiveForm::end(); ?>
		...
		>
			...
			<div class="p-7">
				<div class="mb-4 flex items-center gap-3">
					<div class="h-14 w-14 rounded-full">
						<img
							src=<?= 'uploads/qrisPhoto/' . $userModel->getShop()->qrisimage ?>
							alt="User"/>
					</div>
					<div>
						<span
							class="mb-1.5 font-medium text-black dark:text-white"
						>Edit your Qris Code</span
						>
						<span class="flex gap-2.5">
							<button
								class="text-sm font-medium hover:text-primary"
				...
				<?= $form3->field($shopQrisUploadForm, 'imageFile', [
					'inputOptions' => [
						'class' => 'absolute inset-0 z-50 m-0 h-full w-full cursor-pointer p-0 opacity-0 outline-none',
						'accept' => 'image/*'
					],
